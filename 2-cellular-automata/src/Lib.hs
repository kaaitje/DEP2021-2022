...
-- | Maakt gebruik van de twee voorgaande functies want die pastte precies op elkaar
leftMost = fromList . toList

-- TODO: Schrijf en documenteer de functie rightMost, die de focus helemaal naar rechts opschuift.
-- Voegt recursief de getallen van x toe aan y en stopt als er nog maar 1 in x zit.
rightMost :: FocusList a -> FocusList a
rightMost (FocusList [x] (y:ys)) = (FocusList [x] (y:ys))
rightMost (FocusList (x:xs) ys) = rightMost (FocusList xs (x:ys))
